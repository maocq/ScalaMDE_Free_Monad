...
class ComposingFreeMonads extends FunSuite{
  ...
  class Services[F[_]](implicit I: InjectK[Service, F]) {
    def getUserNameI(userId: Long): Free[F, UserName] = Free.inject[Service, F]( GetUserName( userId ) )
    def getUserPhotoI(userId: Long): Free[F, Image] = Free.inject[Service, F]( GetUserPhoto( userId ) )
  }
  object Services {
  ...
  test("Composing Free monads") {
    ...
    def program(id: Long)(implicit O : Services[CatsApp], L : Logs[CatsApp]): Free[CatsApp, User] = {
      import O._
      import L._
